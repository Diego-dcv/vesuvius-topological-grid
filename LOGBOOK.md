...
## September 2026, week 1 — loader, summary README, wedges at the fine level

- ✔ 06-09 `scripts/pherc1218_io.py`: the missing loading step. Every 1218 script had assumed `rows`, `zs`, `zpos`, `vol` from the notebook; now each loads them itself when run from a terminal (no change inside Colab). Self-check against the README census passes; modes 16, 17 (both scripts) and 18 (labels) re-run from a clean clone and reproduce their published numbers. CT-reading and Kaggle-reading scripts patched the same way, dependencies declared, not re-run here.
- ✔ 06-09 README rebuilt as a summary (309 lines): results others have checked, PHerc. 1218 in numbers with `measured`/`expected` marked, one status table, limits found. The 23 long sections moved unchanged to `docs/modes/` with links fixed. The 5.13 m ribbon relabelled as loom path length; the winding-lane / loom / labelled-length figures agreed with the labels' author added as a mode-16 addendum.
- ✘ 06-09 `cross_scroll_1218.py` read the origins with x and y swapped and without the CT offset (found while writing the loader). The mode-15 results on PHerc. 1218 are flagged on their page as pending a re-run; not re-run yet.
- ✔ 06-09 New experiment written and twin-tested, NOT yet run on the scroll: `wedge_fine_1218.py` — do the wedge sheets reappear at CT level 0 (8.64 µm)? Pre-registered E1/E2a/E2b. The twin (`wedge_fine_twin_1218.py`) caught two design faults before any Colab time: the wedge audit's counter had a 100 µm floor (peak distance 6 vox) so it could not count tight laminae at any resolution — the audit's rho = 0.34 carries that floor; and a physical floor lets noise fake ridges unless prominence is noise-scaled (now 5 sigma per column). Twin verdicts on record in the script docstring.
- ✘ 06-09 `wedge_fine_1218.py` run on Colab (65 min): E1 FAIL — the counter finds half the known sheets at 17 µm (eta 0.54) and over-counts them at 8.6 µm (eta 1.26); E2 not read, nothing tuned. A known sheet may show more than one ridge at the fine level (expected, not measured).
- ✔ 06-09 `planchado_heldout_um_1218.py`: the "control negativo NO – REVISAR" was a nan in the shuffled control (rolled field on empty bins), not a real failure — fixed, control degrades (1954/2232 µm). Sanity R² mismatch explained: the script fitted shape+fold (0.88); mode 17 fitted the fold after removing the slow z trend. Added the mode-17 flavour as sanity (reproduces 0.68) and as predictor P2b (302 µm). Published verdicts unchanged: neighbours 32 µm beat any field by 10x.
- ✔ 06-09 `planchado_1218.py` now writes the artefacts its page lists (`planchado_1218.npz`, `planchado_1218_results.json`); regenerated with the published numbers.
- ✔ 06-09 `cross_scroll_1218.py` (mode 15 on 1218): origins now read by name with the CT offset. The August run used the swapped centre; re-run pending (Colab cell prepared).
- ✔ 07-09 Mode 15 on PHerc. 1218 re-run with the origins read by name and the CT offset: material sanity 88 % → 100 %, result unchanged — no striation band, no fine grain. The August null stands, now measured correctly; fibre texture is not usable on this scroll at 17 µm.
- ✘ 07-09 Fibre-parity marker (`fibre_parity_1218.py`): grain direction is visible at 8.6 µm but does not flip inside a sheet (3 % of 202, chance); it is a property of the place, not the sheet. Diagnostic on the way: 99/120 known sheets have no brightness valley within ±250 µm at 8.6 µm — sheets cannot be delimited by brightness.
- ✔ 07-09 `layer_division_1218.py`: gaps > 1.6× local pitch divided into equal layers put the hidden sheets 35 µm (0.22 pitch) from where the neighbouring plane's labels see them; random 0.46. Count exam failed by a hair (judge sees more sheets than layers in 10.5 %).
- ✘ 07-09 `layer_compression_1218.py`: compression is real in ~1 pack in 4 but flat across angle and depth; a zone correction gains nothing on held-out planes.
- ✘ 08-09 `mass_count_1218.py` (Colab, 3,000 packs): counting sheets by CT mass gives the same as the pitch; material per unit length inside packs 0.93 of free sheets. Packs are not pressed.
- ✔ 08-09 The count excess was split labels: with crossings < 7 vox merged in the judge, "more sheets than layers" drops from 10.5 % to 2.0 % of 240,000 packs. Count exam passes.
- ✔ 08-09 `hidden_sheets_1218.py`: 328,143 predicted sheets in 204,302 packs (`archives/results/for J/hidden_sheets_1218.csv`); 97.6 % on material at 17 µm (labelled sheets 99.4 %). Per plane: labelled 2.57 m + hidden 0.81 m = 3.37 m against a loom of 8.4–9.4 m — packs explain a quarter of the missing papyrus; the rest lies beyond the last labelled sheet on each ray. Written up as mode 20.
- ✘ 08-09 Folds (Diego's surfboard-and-waves observation on the first CT section with the geometry overlaid): where a sheet undulates, one plane cuts it several times, so neighbouring crossings can be one winding. The mode-20 list is crossings, not windings. `fold_check_1218.py` tracks crossings across planes to catch closing pairs; on its twin, 38-60 % of planted waves caught and 7-11 % false — kept as a zone map, not a certifier. The section image and the limit are in the mode-20 page; the question goes to the per-voxel tree. The section image is not posted on Discord.
