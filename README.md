...
# 11. Sheet joins: the kollesis landmarks and their angular chirp
python scripts/synthetic_scroll_twin.py kollesis --columns 95 --plot koll.png

# 12. The falsifiable column map for the real scroll
python scripts/text_layout_predictor.py predict --columns 95 --csv map.csv

# 13. Acceptance tests for both (no arguments, no data required)
python scripts/synthetic_scroll_twin.py test
python scripts/text_layout_predictor.py test
```

Steps 10–13 need no input images: the twin and the predictor run on geometry
alone.
...
glad to be informed.trusted source — never calibrated from the candidate pool, so degraded candidates
cannot contaminate the ruler.
