...
G = dict(
    ...
    kollesis_mm=160.0,   # kollema (sheet) width. MEASURED on a Herculaneum
                         # roll: Philodemus' On Poems II is 16 m over 100
                         # kollemata. This replaces an earlier 180 mm, which
                         # was not a measurement but arithmetic from Pliny's
                         # scapus (20 sheets, 11-12 ft). One roll is not a
                         # universal -- sheet widths varied -- but a measured
                         # Herculaneum value outranks an inference about
...
def kollesis_map(L_mm, g=G):
    ...
    exceeded twenty sheets, about 11-12 feet, which gives sheets of ~17-19 cm
    -- but that is an inference about typical practice, and a measured
    Herculaneum roll supersedes it: Philodemus' On Poems II runs 16 m over
    100 kollemata, i.e. 160 mm per sheet, which is the default here. That
    roll also settles what exceeding a scapus means: it "was at first a roll
    of 70 sheets; a further 30 were glued on when the work proved to be
    long". Gluing scapi together was ordinary, not remarkable.
    Each join is a band of DOUBLE THICKNESS recurring every ~160 mm of arc.
