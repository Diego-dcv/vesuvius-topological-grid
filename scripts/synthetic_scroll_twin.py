...
texture and give the fiber detector its first 3D target. And one falsifiable
external readout: the crushed SECTION SIZE as a function of the work length,
directly comparable with the measured 42 x 21 mm.

NOT: evidence about the real scroll. If the tools recover what the twin
contains, the tools work on the twin's assumptions -- nothing more. The one
statement the twin may make about reality is the section-size curve, because
that compares twin OUTPUT with an INDEPENDENT measurement.

TWO EMERGENT CHECKS (not imposed, they fall out of the geometry)
----------------------------------------------------------------
1. Umbilicus closure: with the measured pitch and section, the spiral closes
   on a ~4 mm umbilicus -- plausible without being imposed.
2. Anisotropic layer gap after crushing: equal-perimeter 2:1 ellipses space
   ~2x wider along the fold axis (~224 um) than along the flattened axis
   (~112 um, BELOW the 173 um nominal pitch). The twin therefore predicts
   merge excess concentrated on the flattened axis -- which is exactly what
   the void-aware run on the real PHerc1218 found, independently. The twin
   was not told this; the geometry says it.
...
G = dict(
    ...
    line_mm=2.79,        # line spacing
    col_period_mm=43.0,  # column period (written + intercolumn)
...
def acceptance_test(verbose=True):
    ...
    C. SELF-CONSISTENCY WITH THE REAL SCROLL. Feed the twin the obra length
       whose section matches the measured 42 x 21 mm; the roll must then show
       ~70 turns (the independently measured winding count).
       PASS: |n_turns - 70| < 3.
       (Section->obra->turns closes a loop through three independent
        measurements: section size, pitch, turn count. If the numbers were
        inconsistent, this fails.)
    """
    results = {}
    ...
    # ---- C -----------------------------------------------------------------
    imp = obra_implied_by_section()
    ok_C = abs(imp['n_turns'] - 70.0) < 3.0
    results['C_consistency'] = dict(**imp, passed=bool(ok_C))
    ...
    if verbose:
        ...
        C = results['C_consistency']
        print(f"C consistency      section 42x21 -> obra {C['n_columns']} cols "
              f"-> {C['n_turns']:.1f} turns (70+-3 measured) -> "
