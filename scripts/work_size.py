...
# Results live in archives/results/ in this repository -- that is the
# convention the README's file tree documents, and where the other generated
# tables sit. The fallbacks exist only so a copy of the script still works
# outside the repo; they are not alternative homes.
#
# The lookup is explicit, and exam D checks it, because a MISSING catalogue
# and an EMPTY one produce identical output while meaning opposite things.
# Reading "no candidates" off a file the tool never found is the failure this
# guards against.
CATALOGUE_CANDIDATES = [
    os.path.join(ROOT, 'archives', 'results', 'roll_catalogue.csv'),  # home
    os.path.join(HERE, 'roll_catalogue.csv'),                # next to script
    os.path.join(ROOT, 'data', 'roll_catalogue.csv'),
    os.path.join(ROOT, 'roll_catalogue.csv'),
]
...
# The prose column period is itself contested, and it is now the DOMINANT
# uncertainty here -- larger than the umbilicus and the winding pitch put
# together, and comparable to not knowing the layout regime at all.
#   43.0 mm  measured on Paris 4 by mode 1, reproducible -- but it implies
#            7.9 letters per line against the ~17 of PHerc. 118, which is
#            arithmetically impossible, and it sits 7 % above the bottom edge
#            of its search band.
#   72.0 mm  what two roll reconstructions give (On Poems II: 16 m / 222
#            columns; PHerc. 1667: ~1.5 m / ~20 columns), and what 4.16 mm x
#            17 letters implies.
# See docs/data_sources.md, "Grid self-consistency".
REGIME_PERIOD_MM = (('prose, period 43 (measured)', 43.0),
                    ('prose, period 72 (roll reconstructions)', 72.0),
                    ('latin verse', 98.0))
...
def acceptance_test(verbose=True):
    ...
    # D
    found = find_catalogue()
    res['D_catalogue_found'] = dict(
        path=(os.path.relpath(found, ROOT) if found else None),
        passed=bool(found))
    ...
    if verbose:
        ...
        D = res['D_catalogue_found']
        where = D['path'] or 'NOWHERE (a missing catalogue reads as empty)'
        print(f"D catalogue found  {where} -> "
...
def main():
    ...
    for r in rows:
        print(f"  {r['regime']:<18} {r['pitch_um']:>6.1f}u {r['r0_mm']:>4.1f}m "
              f"{r['n_columns']:>8d}")
    prose = [r['n_columns'] for r in rows if r['regime'].startswith('prose')]
    verse = [r['n_columns'] for r in rows if r['regime'].startswith('latin verse')]
    p43 = [r['n_columns'] for r in rows if '43' in r['regime']]
    p72 = [r['n_columns'] for r in rows if '72' in r['regime']]
    ...
    print(f"  The prose band is the working answer -- but it is now dominated")
    print(f"  by the COLUMN PERIOD, not by the umbilicus or the winding pitch:\n")
    print(f"    period 43 mm (measured on Paris 4)      ->  "
          f"{min(p43)} - {max(p43)} columns")
    print(f"    period 72 mm (two roll reconstructions) ->  "
          f"{min(p72)} - {max(p72)} columns\n")
    print(f"  43 mm is reproducible but implies 7.9 letters per line against")
    print(f"  the ~17 of PHerc. 118 -- arithmetically impossible. Until that is")
    print(f"  settled, read every column count here as provisional by up to a")
    print(f"  factor of two. See docs/data_sources.md, 'Grid self-consistency'.\n")
    band = (min(prose), max(prose))

    cat_path = find_catalogue()
    if cat_path is None:
        print("  CATALOGUE NOT FOUND (its home is archives/results/).")
        print("  Looked in:")
        for c in CATALOGUE_CANDIDATES:
            print(f"    {os.path.relpath(c, ROOT)}")
        print("  Without it no candidate can be matched OR excluded, and an")
        print("  absent catalogue must not be mistaken for an empty one.")
        return
    print(f"  catalogue: {os.path.relpath(cat_path, ROOT)}")
